...
class player:
    ...
    def change_ability(self, ability_dict , amount):
        for ability, change in ability_dict.items():
            ...
            elif ability == 'Intellect':
                self.attributes['Intellect'] += change

        print(f"Changed {ability} by {ability_dict[ability]} based on roll of {amount}")
        print(f"New {ability} score: {self.attributes[ability]}")
...
class campaign:
    ...
    async def create_dm(cls, username=None, char_name=None):
        ...
        await instance.vb.connect()
        await instance.bvb.connect()
        instance.session_seed = await instance.vb.get_session_id() + 1
        print("Seed: ", instance.session_seed)
        await instance.vb.add_session('all-MiniLM-L6-v2', "start of session", instance.session_seed)
    ...
    def create_campaign_from_file(self, seed):
        r'''
        Needs the seed to decide the campaign to roll with \
        sets the campaign information'''
        
        print("Creating campaign from file with seed:", seed)
